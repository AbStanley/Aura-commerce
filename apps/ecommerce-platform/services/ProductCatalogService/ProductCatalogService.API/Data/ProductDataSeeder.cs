...
public static class ProductDataSeeder
{
    public static async Task SeedAsync(ProductCatalogDbContext context)
    {
        ...
        if (!context.Products.Any())
        {
            ...
            await context.SaveChangesAsync();
            Log.Information("Products seeded.");
        }
        else
        {
            Log.Information("Products already seeded.");
        }

        // Seed Inventory or Restore Stock
        Log.Information("Checking inventory levels...");
        
        var allProducts = context.Products.ToList();
        var allInventories = context.Inventories.ToList();
        ...
        foreach (var product in allProducts)
        {
            ...
            if (inventory == null)
            {
                Log.Information("Creating missing inventory for product {ProductName}", product.Name);
                inventory = Inventory.Create(product.Id, new Random().Next(20, 100));
                context.Inventories.Add(inventory);
                changesMade = true;
            }

            // Special handling for the intended "Out of Stock" item
            if (product.Name.Contains("Mechanical Keyboard"))
            {
                if (inventory.QuantityAvailable != 0)
                {
                    inventory.QuantityAvailable = 0;
                    Log.Information("Setting {ProductName} to Out of Stock (Testing scenario)", product.Name);
                    changesMade = true;
                }
                continue; // Skip the refill logic below
            }

            // Refill stock if it's 0 (or low?) - Let's just fix 0 for now to solve the user's issue
            if (inventory.QuantityAvailable == 0)
            {
                inventory.QuantityAvailable = 50; // Default refill amount
                Log.Information("Restocking {ProductName} - Inventory was 0", product.Name);
                changesMade = true;
            }
        }
        ...
        {
            await context.SaveChangesAsync();
            Log.Information("Inventory updates saved.");
        }
        else
        {
            Log.Information("Inventory levels are healthy.");
        }
